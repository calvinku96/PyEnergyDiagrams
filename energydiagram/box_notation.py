...
def plot_orbital_boxes(ax,x,y,boxes_number, electrons_number, box_side=1, spacing_f=5, priority="up"):
    ...
    if electrons_number > 0:
        directions = ('up', 'down') if priority == 'up' else ('down', 'up')
        if electrons_number <= boxes_number:
            for e in range(electrons_number):
                ax.add_patch(add_spin(Xi+box_side*e,Yi,box_side,direction=directions[0]))
        else:
            for e in range(boxes_number):
                ax.add_patch(add_spin(Xi+box_side*e,Yi,box_side,direction=directions[0]))
            for j in range(electrons_number - boxes_number):
                ax.add_patch(add_spin(Xi+box_side*j,Yi,box_side,direction=directions[1]))
